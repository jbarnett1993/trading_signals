...
resp = LocalTerminal.get_historical(['EURUSD Curncy'], ['PX_LAST'], start='01/01/2020', end='10/26/2020')
df = resp.as_frame()
...
#for a, b in entry_signal.iteritems():
#    if b == -1:
#        plt.plot(a, df['EURUSD Curncy']['PX_LAST'][a], 'rv')
#    else:
#        plt.plot(a, df['EURUSD Curncy']['PX_LAST'][a], 'k^')



"""Read to CSV commented and only used when necessary/saving hammering the API"""
#df.to_csv(r'C:\Users\barnjam\OneDrive - Manulife\trading_signals\CSV Outputs\signaltesting1.csv',index = False)
